...
#pragma HLS INLINE
	...
	void set_pe_weights(T new_weight , bool sw_load)
	{
#pragma HLS INLINE
	...
	void process(T0 new_input,U last_sum , bool sw_calc)
	{
#pragma HLS INLINE
...
#pragma HLS UNROLL //循环展开
    ...
    void set_array_weights(Weight_DataType weight_buffer[WEIGHT_BUFFER_WIDTH][MATRIX_WIDTH],
                          int wb_start_addr ,bool sw_load ,bool enable)
	{
...
#pragma HLS UNROLL //循环展开
    ...
    void BMM_kernel(Input_DataType input_buffer[INPUT_BUFFER_WIDTH][MATRIX_WIDTH],
            Psum_DataType accumulator_buffer[ACCUMULATOR_BUFFER_WIDTH][MATRIX_WIDTH],
            int ib_start_addr,int ob_start_addr, bool sw_calc, bool accumulate, bool enable)
    {
#pragma HLS INLINE recursive
        if (!enable) return;
...
#pragma HLS UNROLL //循环展开
                if (k >= start_index && k <= end_index) {
                    ...
                    U(accumulator_buffer[ob_start_addr+col_index][row_index] + output_vector[row_index]): output_vector[row_index];//写入输出矩阵
                }
            }
        }
    }
...
//BMM函数，调用BMM_kernel函数，实现BMM函数，调用了双缓冲
...
//注意，该函数不包括将input\weight移入缓冲区
//I、K、J代表输入input矩阵、weight矩阵的行列
